...
def ballside_mark_segment(mark_pos, ball_pos = None):
        ...
        # removes radius of ball and 2*radius of robot from segment
        # @param mark_pos: Point (usually robot position) to defend against
        # @param ball_pos: ball position, pulled from main if not given
        # @return: LineSegment to defend on

        #Get ball position if not provided
        if ball_pos is None:
            ball_pos = main.ball().pos

        #Set and return the mark line
